...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.AUTH_ADDRESS}/hashed-password/` + password);  // get evn for 3 way: 1.local service id / 2.env for service name / 3.domain name *** most common way
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTH_ADDRESS}/token/` + hashedPassword + '/' + password  // get evn for 3 way: 1.local service id / 2.env for service name / 3.domain name *** most common way
  );
